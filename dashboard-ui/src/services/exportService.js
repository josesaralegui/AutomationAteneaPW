...
export const exportToExcel = (report) => {
    ...
    XLSX.utils.sheet_add_aoa(ws, [['Total Tests', 'Passed', 'Failed', 'Flaky', 'Pass Rate']], {
        origin: 'A8',
    });
    ...
    const passRate =
        totalTests > 0 ? Math.round(((report?.stats?.expected || 0) / totalTests) * 100) : 0;
    ...
    (report?.suites || []).forEach((suite) => {
        (suite.specs || []).forEach((spec) => {
            ...
            if (test && test.results && test.results.length) {
                ...
                XLSX.utils.sheet_add_aoa(ws, [[status, testName, suiteName, duration, startTime]], {
                    origin: `A${rowIdx}`,
                });
    ...
    if (yPosition + 40 < pageHeight - 20) {
        ...
        autoTable(doc, {
            ...
            didParseCell: (data) => {
                ...
                const rawStatus =
                    data.row.raw && data.row.raw[0] ? data.row.raw[0].toString().toUpperCase() : '';
